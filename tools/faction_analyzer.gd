...
var faction_signatures: Dictionary = {}  # entry_name -> PackedFloat64Array (for similarity)
...
func _analyze_emoji_distribution():
	...
	for faction in factions:
		var entry_name = faction.get("entry_name", "Unknown")
		...
		for emoji in signature:
			...
			emoji_usage[emoji]["factions"].append(entry_name)
...
func _compute_faction_similarities():
	...
	var faction_sets: Dictionary = {}  # entry_name -> Set of emojis
	for faction in factions:
		var entry_name = faction.get("entry_name", "Unknown")
		var signature = faction.get("signature", [])
		var hamiltonian = faction.get("hamiltonian", {})
		...
		faction_sets[entry_name] = emoji_set
	...
	for entry_name in names:
		...
		for sim in similarities:
			if (sim["faction_a"] == entry_name or sim["faction_b"] == entry_name) and sim["jaccard"] > 0:
				has_overlap = true
				break
		if not has_overlap:
			isolated += 1
