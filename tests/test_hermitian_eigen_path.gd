...
func test_eigen_backend_is_the_only_backend():
	...
	_check("summary returned", not summary.is_empty())
	if summary.get("eigenvalues", PackedFloat64Array()).is_empty():
		...
		_check("SKIP: Hamiltonian empty, solver not reached", true)
		return
	_check("used_external_backend is true (Eigen ran)",
		summary.get("used_external_backend", false) == true)
	_check("eigenvectors_available is true",
		summary.get("eigenvectors_available", false) == true)
	var evs: PackedFloat64Array = summary["eigenvalues"]
	var all_finite := true
	for v in evs:
		if is_nan(v) or is_inf(v):
			all_finite = false
	_check("eigenvalues all finite (%d values)" % evs.size(), all_finite)
	# A Hermitian matrix has real eigenvalues and Eigen returns them ascending.
	var sorted := true
	for i in range(1, evs.size()):
		if evs[i] < evs[i - 1] - 1e-9:
			sorted = false
	_check("eigenvalues ascending (Eigen convention)", sorted, str(evs))
...
func test_complex_coherence_changes_the_spectrum():
	...
	_check("purity is finite before (%s)" % w_before, not is_nan(w_before))
	_check("purity is finite after (%s)" % w_after, not is_nan(w_after))
	_check("a purely imaginary kick moved the state (Δpurity=%s)" % abs(w_after - w_before),
		abs(w_after - w_before) > 1e-12,
		"imaginary part appears to be discarded — real-projection solve?")
	...
	_check("principal mode resolves before", not m_before.is_empty())
	_check("principal mode resolves after", not m_after.is_empty())
	var ev_b: float = float(m_before.get("eigenvalue", 0.0))
	var ev_a: float = float(m_after.get("eigenvalue", 0.0))
	_check("principal eigenvalue finite (%s → %s)" % [ev_b, ev_a],
		not is_nan(ev_b) and not is_nan(ev_a))
...
func test_principal_mode_is_well_formed():
	...
	_check("mode returned", not mode.is_empty(), str(mode))
	if mode.is_empty():
		return

	var eigenvalue: float = float(mode.get("eigenvalue", -1.0))
	_check("dominant eigenvalue in [0,1] (got %s)" % eigenvalue,
		eigenvalue >= -1e-9 and eigenvalue <= 1.0 + 1e-9)
	...
	if n > 0:
		_check("dominant ≥ 1/n (%s ≥ %s)" % [eigenvalue, 1.0 / float(n)],
			eigenvalue >= 1.0 / float(n) - 1e-9,
			"looks like the smallest eigenvalue was taken instead")
	...
	for a in axes:
		if is_nan(a) or is_inf(a):
			finite = false
	_check("axis projection all finite (%d axes)" % axes.size(), finite)

	var purity: float = e.faction_purity()
	_check("purity in [0,1] (got %s)" % purity, purity >= -1e-9 and purity <= 1.0 + 1e-9)
