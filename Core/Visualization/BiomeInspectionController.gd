...
static func get_biome_data(biome: BiomeBase, _farm_grid) -> Dictionary:
...
static func get_emoji_energy_distribution(biome: BiomeBase) -> Array[Dictionary]:
	...
	# If cache not ready, do not invent probabilities.
	if total_prob <= 0.0:
		return emoji_data
	...
	for emoji in biome.producible_emojis:
		...
		emoji_data.append({
			...
			"trend": "stable"  # needs energy history to compute; see _calculate_energy_flow_rate
		})
...
static func get_lindblad_transfers(_biome: BiomeBase) -> Array[Dictionary]:
...
static func _get_biome_name(biome: BiomeBase) -> String:
	# Extract biome name via get_biome_type() (canonical on all biomes).
...
static func _count_active_projections(_biome: BiomeBase) -> int:
	return 0
...
static func _get_emoji_probability(biome: BiomeBase, emoji: String) -> float:
	...
	var p = snap.get("p0", -1.0) if pole == 0 else snap.get("p1", -1.0)
	return clampf(p, 0.0, 1.0) if p >= 0.0 else -1.0
...
static func _calculate_energy_flow_rate(_biome: BiomeBase) -> float:
	# Calculate dE/dt (energy derivative)

	# Returns rate of energy change in bath (⚡/s)
	# No energy history stored; caller gets 0 until history tracking is added.
	return 0.0
...
static func get_quantum_detail(biome: BiomeBase) -> Dictionary:
	...
	return {
		...
		"basis_states": _get_basis_states(biome),
	}
...
static func _get_qubit_axes(biome: BiomeBase) -> Array:
	...
	for qubit_idx in range(viz.get_num_qubits()):
		...
		if not bloch.is_empty():
			p_north = bloch.get("p0", -1.0)
			p_south = bloch.get("p1", -1.0)
		...
		var balance = (p_north - p_south) / total if total > 0.0 else -1.0
...
static func _get_basis_states(biome: BiomeBase) -> Array:
	# Get computational basis state probabilities from QuantumComputer.

	# Returns array of {label: String, probability: float} sorted descending.
	if not biome or not "quantum_computer" in biome or not biome.quantum_computer:
		return []
	if not biome.quantum_computer.has_method("get_basis_state_probabilities"):
		return []
	return biome.quantum_computer.get_basis_state_probabilities()
