...
static func _load_if_needed() -> void:
	...
	VerboseHelper.info("biome", "profile", "Loaded characteristics for %d biomes" % biome_count)
...
static func get_max_continuous_dt(biome_name: String) -> float:
	...
	# Skipped biomes keep the default minimum dt.
...
static func apply_to_biome(biome) -> void:
	...
	if "max_evolution_dt" in biome:
		...
		VerboseHelper.info("biome", "profile", "Set %s.max_evolution_dt = %.6f" % [biome_name, optimal_dt])
