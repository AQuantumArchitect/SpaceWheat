...
**Source file:** archived concept. The live inside-facing path is `UI/Core/QuantumInstrumentInput.gd` plus the rig listener's input helpers.
...
- `time_skip` and `victory_lap_partial` are explicitly listed as unsupported — they require direct API calls or headless backend.
...
## Connections
- **QuantumInstrumentInput** — the real input handler for headed keyboard play. The rig listener now drives it directly via `_press_key` and navigation helpers.
- **ToolConfig** — `FRAME_ACE`, `FRAME_MERCHANT`, `FRAME_CAPTAIN` constants define the action → archetype frame mapping.
- **Rig/rig_listener.gd** — the direct consumer; the live listener now owns the turn orchestration and input routing helpers.
