...
4. **Confrontation:** Three paths to resolution:
   - **Harvest hard:** Decohere the tomato clique before critical mass (Scientist frame)
   - **Negotiate:** Offer them their own biome slot; they stop colonizing yours (Socialite frame)
...
- A `TomatoClique` faction-biome with `gated_lindblad` couplings pointing INTO other biomes
...
- The clique "biome" runs its own Hamiltonian, injecting cross-biome terms via
