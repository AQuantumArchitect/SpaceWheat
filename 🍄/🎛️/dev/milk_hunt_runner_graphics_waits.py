...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Single-run milk hunt rig player with graphics pacing waits")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "config" / "milk_hunt_graphics_waits.json",
        help="Graphics runner config JSON path",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Sleep after each non-probe turn (graphics pacing)",
    )
    parser.add_argument(
        "--probe-delay-s",
        type=float,
        default=None,
        help="Sleep after probe_cycle turns (graphics pacing)",
    )
    parser.add_argument(
        "--probe-each-loop",
        ...
        action="store_true",
        default=None,
        help="Drive graphics updates each loop via probe_cycle",
    )
    parser.add_argument(
        "--no-probe-each-loop",
        dest="probe_each_loop",
...
def main() -> int:
    ...
    probe_each_loop = args.probe_each_loop
    ...
    if turn_delay_s is None:
        turn_delay_s = get_cfg_float(cfg, "turn_delay_s")
    if turn_delay_s is None and os.environ.get("MILK_HUNT_TURN_DELAY_S", "") != "":
        turn_delay_s = float(os.environ["MILK_HUNT_TURN_DELAY_S"])
    if turn_delay_s is None:
        turn_delay_s = 0.10

    if probe_delay_s is None:
        probe_delay_s = get_cfg_float(cfg, "probe_delay_s")
    if probe_delay_s is None and os.environ.get("MILK_HUNT_PROBE_DELAY_S", "") != "":
        probe_delay_s = float(os.environ["MILK_HUNT_PROBE_DELAY_S"])
    if probe_delay_s is None:
        probe_delay_s = 0.25

    if grid_refresh_every is None:
        grid_refresh_every = get_cfg_int(cfg, "grid_refresh_every")
    if grid_refresh_every is None:
        grid_refresh_every = 5

    cfg_probe_each_loop = get_cfg_bool(cfg, "probe_each_loop")
    if probe_each_loop is None and cfg_probe_each_loop is not None:
        probe_each_loop = cfg_probe_each_loop
    if probe_each_loop is None:
        probe_each_loop = True

    TURN_DELAY_S = max(0.0, float(turn_delay_s))
    PROBE_DELAY_S = max(0.0, float(probe_delay_s))
    if load_alias:
        load_slot = None
    ...
    if not args.reuse_listener:
        ...
        if not args.no_clear_rig:
            _clear_rig_files()
        proc = _start_listener(load_slot=load_slot, scenario_id=scenario_id)
    ...
    try:
        ...
        if isinstance(resources, dict) and not strict_biome_economy:
            for emoji in ["👥", "🌾", "🍞", "❄️", "🌱", "⚙", "🔥"]:
        ...
        for loop_idx in range(max_loops):
            if probe_each_loop:
                if loop_idx % max(1, int(grid_refresh_every)) == 0:
                    history.append(_run_turn(turn, "grid_snapshot"))
                    turn += 1
        ...
        summary = {
            ...
            "grid_refresh_every": grid_refresh_every,
        }
