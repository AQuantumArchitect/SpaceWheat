...
def main():
    ...
    try:
        ...
        # GENUINE 🔨 grind via the KEYBOARD market — no add_resource. Loop accept→complete
        # until we can afford the plant (13 🔨 + 5 🌱). 🍞 self-replenishes via Village.
        print("\n-- keyboard 🔨 grind (need 13) --  start 🔨=%d 🍞=%d 🌱=%d" % (
            res().get("🔨", 0), res().get("🍞", 0), res().get("🌱", 0)))
        grind_cycles = 0
        for mr in range(1, 31):
            ...
            press("ESCAPE", settle=3)
            r = res()
            print("   [grind%d] 🔨=%d 🍞=%d 🌱=%d" % (mr, r.get("🔨", 0), r.get("🍞", 0), r.get("🌱", 0)))
            if r.get("🔨", 0) >= 13 and r.get("🌱", 0) >= 5:
                print(f"   ✓ affordable after {mr} keyboard grind cycles"); break
